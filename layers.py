...
class Dense(Layer):
    ...
    def initialize(self):
        ...
        self.weights = np.random.uniform(-limit, limit, (self.input_shape[0], self.units))
...
class Conv2D(Layer):
    ...
    def initialize(self):
        ...
        self.weights = np.random.uniform(-limit, limit, (self.filters, channels, kernel_height, kernel_width))
    ...
    def img_to_col_indices(self) -> tuple[NDArray[float64], NDArray[float64], NDArray[float64]]:
        ...
        i0 = np.tile(np.repeat(np.arange(kernel_height), kernel_width), channels)
    ...
    def img_to_col(self, images: NDArray[float64]) -> NDArray[float64]:
        ...
        images_padded = np.pad(images, ((0, 0), (0, 0), self.pad_h, self.pad_w))
    ...
    def col_to_img(self, cols: NDArray[float64]) -> NDArray[float64]:
        ...
        images_padded = np.zeros((self.batch_size, channels, height_padded, width_padded))
        
        k, i, j = self.img_to_col_indices()
        
        kernel_height, kernel_width = self.kernel_shape
        cols = cols.reshape(channels * kernel_height * kernel_width, -1, self.batch_size).transpose(2, 0, 1)
    ...
    def backward(self, output_gradient, training=True):
        output_gradient_col = output_gradient.transpose(1, 2, 3, 0).reshape(self.filters, -1)
        
        if training:
            self.weights_gradient = (output_gradient_col @ self.input_col.T).reshape(self.weights.shape)
            self.bias_gradient = np.sum(output_gradient_col, axis=1, keepdims=True)
    ...
    def get_output_shape(self):
        ...
        output_height: int = (input_height - kernel_height + np.sum(self.pad_h)) // self.stride + 1
        output_width: int = (input_width - kernel_width + np.sum(self.pad_w)) // self.stride + 1
...
class MaxPooling2D(Layer):
    ...
    def backward(self, output_gradient, training = True):
        ...
        output_gradient = output_gradient.reshape(batch_size, channels, output_height, 1, output_width, 1)
    ...
    def get_output_shape(self):
        ...
        height = (input_height + sum(self.pad_h) - pool_height) // self.stride + 1
...
class Dropout(Layer):
    ...
    def forward(self, input_value, training=True):
        if training:
            self.mask = self.scale * (np.random.uniform(0, 1, input_value.shape) > self.p)
        else:
            self.mask = 1
...
class BatchNormalization(Layer):
    ...
    def backward(self, output_gradient, training=True):
        ...
        if training:
            self.weights_gradient = np.sum(self.input_normalized * output_gradient, axis=0)
